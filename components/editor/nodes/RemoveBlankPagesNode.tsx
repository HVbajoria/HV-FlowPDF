...
import { FileMinus, Play } from 'lucide-react';
import { useFlowStore } from '../../../store/flowStore';
import { removeBlankPages } from '../../../lib/pdf-processing';

export const RemoveBlankPagesNode = memo(({ data }: { data: any }) => {
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      const newPdfBuffer = await removeBlankPages(file.data);
      
      const newFileId = `noblank-${Date.now()}`;
  ...
  return (
    <Card className="w-[350px] shadow-lg border-border/60 bg-card/95 backdrop-blur-sm relative">
      ...
      <CardContent className="p-5 pt-2 space-y-4">
        <p className="text-xs text-muted-foreground">Automatically detects and removes blank pages.</p>
        <Button 
            className="w-full" 
        ...
        >
            {isProcessing ? 'Processing...' : <><Play className="w-4 h-4 mr-2" /> Run</>}
        </Button>
        {data.fileName && (
...
RemoveBlankPagesNode.displayName = 'RemoveBlankPagesNode';